...
```
Is there GO enrichment in these unique gene sets?
...
hiz2_2_sites <- ggplot(df2, aes(x = mean_percent_modified_col0,
               y = mean_percent_modified_mutant,
               color = higher_in_x,
               size = mean_Nvalid_cov)) +
  geom_point(alpha = 0.8, show.legend = TRUE) +  
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "grey50") +
  scale_color_manual(values = c("FALSE" = "#CCAA22", "TRUE" = "grey50")) +
...
hiz2_4_sites <- ggplot(df, aes(x = mean_percent_modified_col0,
...
```
Count up the sites where one is higher than the other?
hiz2-2
```{r}
count_non_na <- summary_table_means %>%
  filter(
    !is.na(mean_percent_modified_mutant),
    !is.na(mean_percent_modified_col0),
    !(mean_percent_modified_mutant == 0 & mean_percent_modified_col0 == 0)
  ) %>%
  nrow()

count_higher <- summary_table_means %>%
  filter(
    !is.na(mean_percent_modified_mutant),
    !is.na(mean_percent_modified_col0),
    !(mean_percent_modified_mutant == 0 & mean_percent_modified_col0 == 0),
    mean_percent_modified_mutant > mean_percent_modified_col0
  ) %>%
  nrow()

count_lower <- summary_table_means %>%
  filter(
    !is.na(mean_percent_modified_mutant),
    !is.na(mean_percent_modified_col0),
    !(mean_percent_modified_mutant == 0 & mean_percent_modified_col0 == 0),
...
count_equal <- summary_table_means %>%
  filter(
    !is.na(mean_percent_modified_mutant),
    !is.na(mean_percent_modified_col0),
    !(mean_percent_modified_mutant == 0 & mean_percent_modified_col0 == 0),
  ...
  nrow()
count_non_na
count_higher / count_non_na * 100
count_lower / count_non_na * 100
count_equal / count_non_na * 100
```
hiz2-4
```{r}
count_non_na <- summary_table_means %>%
  filter(
    !is.na(mean_percent_modified_OX),
    !is.na(mean_percent_modified_col0),
    !(mean_percent_modified_OX == 0 & mean_percent_modified_col0 == 0)
  ) %>%
  nrow()
...
count_higher <- summary_table_means %>%
  filter(
    ...
    !is.na(mean_percent_modified_col0),
    !(mean_percent_modified_OX == 0 & mean_percent_modified_col0 == 0),
    mean_percent_modified_OX > mean_percent_modified_col0
  ) %>%
  nrow()
