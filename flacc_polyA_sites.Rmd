...
hiz2_4_col0_res <- read.table("~/Downloads/flacc_vs_col0.apa_results.bed")
hiz2_4_col0_res.2 <- read.table("/Volumes/cluster/ggs_lab/cmetheringham001/SIKFLACC/sikflacc_nanopore/data/apa_results/flacc_OX_vs_col0.apa_results.bed")
hiz2_2_col0_res <- read.table("/Volumes/cluster/ggs_lab/cmetheringham001/SIKFLACC/sikflacc_nanopore/data/apa_results/flacc_mutant_vs_col0.apa_results.bed")
colnames(vir1_virc_res) <- colnames(vir1_col0_res) <- colnames(hiz2_4_col0_res) <- colnames(hiz2_4_col0_res.2) <- colnames(hiz2_2_col0_res) <- results_header
```
...
```
```{r}
hiz2_4_col0_res.filter <- hiz2_4_col0_res.2[which(hiz2_4_col0_res.2$wass_dist > 25 & hiz2_4_col0_res.2$wass_fdr < 0.05),]
...
```
```{r}
hiz2_2_col0_res.filter <- hiz2_2_col0_res[which(hiz2_2_col0_res$wass_dist > 25 & hiz2_2_col0_res$wass_fdr < 0.05),]

hiz2_2_site_choice <- ggplot(hiz2_2_col0_res.filter, aes(x=wass_dir))+
    scale_x_continuous(limits = c(-250,250), expand = c(0, 0)) +
    scale_y_continuous(expand = c(0, 0)) +
    ...
    geom_histogram(fill = "#CCAA22", colour="white", bins=30)+
    annotate("text", x = -200, y = 140, label = paste("n=", nrow(hiz2_2_col0_res.filter)), hjust = 1, size = 3) +
    theme_classic(base_size=10)+
    ylab("frequency")+
    xlab("Change in mean 3' position")+
        geom_vline(xintercept=0,linetype="dashed")
...
```{r}
ggsave(
  filename = "../../Figures/figure_2/hiz2_2_site_choice.pdf",
  plot = hiz2_2_site_choice,
  width = 6, height = 5, units = "in" 
)
ggsave(
  filename = "../../Figures/figure_2/hiz2_2_site_choice.png",
  plot = hiz2_2_site_choice,
  width = 6, height = 5, units = "in" 
)
```
